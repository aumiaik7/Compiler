...
\huge Technical Documentation of Compiler construction\\ % The assignment title
...
From the code we see a new method NewLabel(). It simply generates and returns a new label every time it is called and the label is used as address, sometimes assign variable length where necessary.\\

We modified the method of parser and added admin.emit1(string), admin.emit2(string,int), admin.emit3(string,int,int) where applicable. Finally if our code is free from any error we get our intermediate machine code written in a file. The file pointer is then passed to the Assembler. Using the intermediate code we perform 2 passes in Assembler and get assembly code for interpreter. Using the machine code Interpreter executes the machine code which is equivalent to our PL source code.\\ 
